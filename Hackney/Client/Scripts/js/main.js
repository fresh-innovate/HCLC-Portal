jQuery(document).ready(function($){
	...
	********************/
	...
	/*******************
		icons
	********************/
	var iconsWrapper = $('#icons .cd-box'),
		iconsHtml = iconsWrapper.html(),
		containerHtml = $('<div class="row cd-box"></div>').insertAfter(iconsWrapper),
		iconsHtmlText = iconsHtml.split('<div class="fa-hover col-md-3 col-sm-4">');

	$.map(iconsHtmlText, function(value){
		if(value.indexOf('span') >= 0 ) {
			var segments = value.split('><');

			var wrapperElement = $('<p></p>').text("<"+segments[1]+"></i>");
			wrapperElement.appendTo(containerHtml);
		}
	});	
	...
	/*******************
		form
	********************/
	var formWrapper = $('#form .cd-box'),
		formHtml = formWrapper.html(),
		containerHtml = $('<div class="cd-box"></div>').insertAfter(formWrapper),
		formHtmlText = formHtml;

	var wrapperElement = $('<pre></pre>').text(formHtmlText);
	wrapperElement.appendTo(containerHtml);
